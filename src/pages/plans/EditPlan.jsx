    ...
    useEffect(() => {
        const loadPlan = async () => {
            ...
            try {
                ...
                const standardIds = STANDARD_FEATURES.map(f => f.id);

                const foundStandard = loadedFeatures.filter(f => standardIds.includes(f));
                const foundCustom = loadedFeatures.filter(f => !standardIds.includes(f));

                setSelectedFeatures(foundStandard);
                setCustomFeatures(foundCustom.join(', '));
            } catch (error) {
        ...
        loadPlan();
    }, [planId, navigate, setValue, toast]);
